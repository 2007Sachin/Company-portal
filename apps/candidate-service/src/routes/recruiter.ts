...
recruiterRouter.get('/me/requests', async (req: Request, res: Response): Promise<void> => {
  try {
...
recruiterRouter.put('/me/requests/:id/respond', async (req: Request, res: Response): Promise<void> => {
  try {
    const user = getUser(req);
    const { id } = req.params;
...
recruiterRouter.post('/me/signals', async (req: Request, res: Response): Promise<void> => {
  try {
    const user = getUser(req);
    const parsed = signalSchema.safeParse(req.body);
    if (!parsed.success) { res.status(400).json({ error: (parsed as any).error.issues }); return; }
    ...
    if (interest_type === 'viewed') {
       ...
       if (existing) { res.json({ status: 'ignored_duplicate' }); return; }
    }
