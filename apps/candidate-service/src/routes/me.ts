...
meRouter.get('/interests', async (req: Request, res: Response): Promise<void> => {
  try {
    const user = getUser(req);
    const supabase = getSupabase();
    ...
    if (error) { res.status(400).json({ error: (error as any).message }); return; }
    res.json(data);
  } catch (err) {
    res.status(500).json({ error: 'Internal server error' });
  }
...
meRouter.put('/interests/:id/respond', async (req: Request, res: Response): Promise<void> => {
  try {
    const user = getUser(req);
    const { id } = req.params;
...
meRouter.put('/matches/:id/save', async (req: Request, res: Response): Promise<void> => {
  try {
    const user = getUser(req);
    const { id } = req.params;
...
meRouter.put('/matches/:id/dismiss', async (req: Request, res: Response): Promise<void> => {
  try {
    const user = getUser(req);
    const { id } = req.params;
    const supabase = getSupabase();
...
meRouter.post('/opportunities/recompute', async (req: Request, res: Response): Promise<void> => {
  try {
    const user = getUser(req);
    const supabase = getSupabase();
    ...
    if (jdsError) { res.status(500).json({ error: jdsError.message }); return; }
    ...
    if (!copRes.ok) throw new Error('Copilot ranking failed');
    const rankedData = (await copRes.json() as any).ranked;

    const matchesToUpsert = rankedData
      .filter((r: any) => r.match_score >= 60)
      .map((r: any) => ({
        candidate_id: user.id,
        jd_id: r.jd_id,
...
meRouter.post('/interests', async (req: Request, res: Response): Promise<void> => {
  try {
    const user = getUser(req);
    ...
    if (error) {
       ...
       if (error.code === '23505') { res.status(409).json({ error: 'Already applied' }); return; }
       res.status(400).json({ error: error.message });
       return;
    }
...
meRouter.put('/goals', async (req: Request, res: Response): Promise<void> => {
  try {
    const user = getUser(req);
...
meRouter.post('/github/scan', async (req: Request, res: Response): Promise<void> => {
  try {
    const user = getUser(req);
    ...
    const processedRepos = await Promise.all(githubRepos.map(async (repo) => {
      ...
      Object.keys(languages as object).forEach(l => inferredSkills.add(l));
      ...
      return {
        ...
        inferred_skills: Object.keys(languages as object),
...
meRouter.post('/storage/sign', async (req: Request, res: Response): Promise<void> => {
  try {
    const user = getUser(req);
...
meRouter.get('/github-repos', async (req: Request, res: Response) => {
  try {
    const user = getUser(req);
    const supabase = getSupabase();
    const { data, error } = await supabase
...
meRouter.put('/github-repos/:id', async (req: Request, res: Response) => {
  try {
    const user = getUser(req);
    const { id } = req.params;
...
meRouter.get('/case-studies', async (req: Request, res: Response) => {
  try {
    const user = getUser(req);
    const supabase = getSupabase();
    const { data, error } = await supabase
...
meRouter.post('/case-studies', async (req: Request, res: Response) => {
  try {
    const user = getUser(req);
    const supabase = getSupabase();
    const { data, error } = await supabase
...
meRouter.delete('/case-studies/:id', async (req: Request, res: Response) => {
  try {
    const user = getUser(req);
    const { id } = req.params;
    const supabase = getSupabase();
...
meRouter.get('/mock-interviews', async (req: Request, res: Response) => {
  try {
    const user = getUser(req);
    const supabase = getSupabase();
    const { data, error } = await supabase
...
meRouter.post('/mock-interviews', async (req: Request, res: Response) => {
  try {
    const user = getUser(req);
    const supabase = getSupabase();
    const { data, error } = await supabase
...
meRouter.put('/mock-interviews/:id/share', async (req: Request, res: Response) => {
  try {
    const user = getUser(req);
    const { id } = req.params;
...
meRouter.get('/skill-assessments', async (req: Request, res: Response) => {
  try {
    const user = getUser(req);
    const supabase = getSupabase();
    const { data, error } = await supabase
...
meRouter.post('/skill-assessments', async (req: Request, res: Response) => {
  try {
    const user = getUser(req);
    const supabase = getSupabase();
    const { data, error } = await supabase
...
meRouter.get('/video-pitch', async (req: Request, res: Response) => {
  try {
    const user = getUser(req);
    const supabase = getSupabase();
    const { data, error } = await supabase
...
meRouter.post('/video-pitch', async (req: Request, res: Response) => {
  try {
    const user = getUser(req);
    const supabase = getSupabase();
    const { data, error } = await supabase
