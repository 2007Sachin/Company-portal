...
function normalizeTextArray(values: any): string[] {
...
export async function fetchCandidateContext(candidateId: string) {
...
export function buildScoreBreakdownFromContext(context: Awaited<ReturnType<typeof fetchCandidateContext>>) {
  ...
  const commits90d = repos.reduce((sum: number, repo: any) => sum + Number(repo.commit_count_30d || 0) * 3, 0);
  const totalStars = repos.reduce((sum: number, repo: any) => sum + Number(repo.stars || 0), 0);
  const languageCount = new Set(
    repos.flatMap((repo: any) => normalizeTextArray(repo.inferred_skills))
  ).size;
  const readmeCoverage = repos.filter((repo: any) => repo.ai_generated_readme).length;

  const githubVelocity = round(
    clamp((commits90d / 60) * 12, 0, 12) +
      clamp((totalStars / 20) * 5, 0, 5) +
      clamp((languageCount / 6) * 4, 0, 4) +
      clamp((readmeCoverage / 3) * 4, 0, 4),
    1
  );

  const weightedSolved =
    Number(candidate.leetcode_easy_solved || 0) * 1 +
    Number(candidate.leetcode_medium_solved || 0) * 2 +
    Number(candidate.leetcode_hard_solved || 0) * 4;
  const problemSolving = round(
    clamp((weightedSolved / 180) * 20, 0, 20) +
      clamp((Number(candidate.leetcode_contest_rating || 0) / 2200) * 5, 0, 5),
    1
  );
  ...
  const recentActivityCount = context.activityEvents.filter((event: any) => new Date(event.created_at).getTime() >= new Date(daysAgo(30)).getTime()).length;
  const consistency = round(
    clamp((Number(streak.current_streak || 0) / 14) * 7, 0, 7) +
      clamp((commits90d / 45) * 4, 0, 4) +
      clamp((recentActivityCount / 12) * 4, 0, 4),
    1
  );

  const acceptedAgentActions = agentActions.filter((action: any) => action.status === 'completed').length;
  const engagement = round(
    clamp((mockInterviews.length / 4) * 6, 0, 6) +
      clamp((acceptedAgentActions / 6) * 5, 0, 5) +
      clamp((recentActivityCount / 10) * 4, 0, 4),
    1
  );

  const overall = round(
    clamp(githubVelocity, 0, 25) +
      clamp(problemSolving, 0, 25) +
      clamp(profileCompleteness, 0, 20) +
      clamp(consistency, 0, 15) +
      clamp(engagement, 0, 15),
    1
  );
...
export function buildScoreCoach(context: Awaited<ReturnType<typeof fetchCandidateContext>>) {
  const candidate = context.candidate ?? {};
  const actions: Array<{ id: string; title: string; estimate_gain: number; effort: number; href: string; description: string }> = [];
...
export async function recalculatePulseScoreV2(candidateId: string) {
  const supabase = getSupabase();
...
export async function recordActivityEvent(candidateId: string, eventType: string, eventData: Record<string, any>) {
  const supabase = getSupabase();
...
export async function buildVisibilitySummary(candidateId: string) {
  const supabase = getSupabase();
  const context = await fetchCandidateContext(candidateId);
  const candidate = context.candidate ?? {};
  ...
  const searchesThisWeek = context.searchAppearances.filter(
    (appearance: any) => new Date(appearance.appeared_at).getTime() >= new Date(daysAgo(7)).getTime()
  ).length;
  const profileViewsThisWeek = context.profileViews.filter(
    (view: any) => new Date(view.viewed_at).getTime() >= new Date(daysAgo(7)).getTime()
  ).length;
  ...
  if (candidateSkill && city) {
    ...
    const sameMarket = (peers ?? []).filter(
      (peer: any) => (peer.city || peer.location || '').toLowerCase() === String(city).toLowerCase()
    );

    if (sameMarket.length > 0) {
      const sorted = sameMarket
        .map((peer: any) => Number(peer.pulse_score || 0))
  ...
  context.profileViews.forEach((view: any) => {
...
export async function listActivityFeed(candidateId: string, limit = 10) {
  const supabase = getSupabase();
...
function computeExperienceFit(candidateYears: number, jdYears: number) {
  if (jdYears <= 0) return 100;
  if (candidateYears >= jdYears) return 100;
  if (candidateYears + 1 >= jdYears) return 80;
  if (candidateYears + 2 >= jdYears) return 55;
  return 20;
}

function computePreferenceAlignment(candidate: CandidateRecord, goals: CandidateRecord, jd: CandidateRecord) {
  let score = 40;
...
export async function computeMatchesForCandidate(candidateId: string) {
  const supabase = getSupabase();
  const context = await fetchCandidateContext(candidateId);
  const candidate = context.candidate ?? {};
  const goals = context.goals ?? {};
  ...
  const recentActivity = Number(candidate.last_active_at) || context.activityEvents.some((event: any) => new Date(event.created_at).getTime() >= new Date(daysAgo(7)).getTime()) ? 100 : 30;

  const matches = (jds ?? []).map((jd: any) => {
...
export async function getOrCreateDailyChallenge(candidateId: string) {
  const supabase = getSupabase();
